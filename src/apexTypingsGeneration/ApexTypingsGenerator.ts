...
export class ApexTypingsGenerator implements IApexTypingsGenerator{
	...
	async generateTypingsForPath(path: string, typingsFolder: string): Promise<any> {
	...
	async generate(namespace:string | null, className:string, typingsFolder:string, parsedClass) {
		const typings = await Promise.all([
			this.apexClassTypingsGenerator.generateClassTypings(namespace, className, parsedClass),
			this.wiredMethodsTypingsGenerator.generateWiredMethodsTypings(namespace, className,parsedClass),
