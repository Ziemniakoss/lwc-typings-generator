...
export default class GenerateLabelsTypings extends SfdxCommand {
	...
	async run() {
		...
		const labelsFullNames = await this.org.getConnection()
			.metadata
			.list({type: "CustomLabel"})
			.then(labelsMetadata => labelsMetadata.map(labelMetadata => labelMetadata.fullName))
		let typings= ""
		for (const fullName of labelsFullNames) {
			this.ux.setSpinnerStatus(fullName)
			const labelMetadata = await this.org.getConnection()
				.metadata
				.read("CustomLabel", fullName)
...
typingsDir,
	...
	generateTypingsForLabelMetadata(labelMetadata):string {
		...
		+ "\n*/\n"
		return `declare module "@salesforce/label/${namespace}.${fullName}" {\n`
		+ docsString
