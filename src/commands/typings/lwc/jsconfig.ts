...
export default class GenerateJsConfigs extends SfdxCommand{
	...
	async run() {
		...
		const lwcMetadataFilesPaths = await findAllFilesWithExtension(this.project.getPath(), LWC_METADATA_FILE_EXTENSION)
		...
		for(const lwcMetadataFile of lwcMetadataFilesPaths) {
			...
			await jsConfigGenerator.generateJsConfig(lwcMetadataFile, lwcMetadataFilesPaths, stdlibPath)
