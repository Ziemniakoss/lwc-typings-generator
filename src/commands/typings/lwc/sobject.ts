...
export default class GenerateSObjectTypings extends SfdxCommand {
	...
	async run() {
		...
		await Promise.all([
			this.generateSchemas(sObjects, schemaGenerator, typingsFolder, describesMap),
			this.generateTypings(sObjects, sObjectTypingsGenerator, typingsFolder, describesMap),
	...
	async generateHelperSchemaTypes(typingsFolder:string) {
		...
		await promises.writeFile(fullPath, "declare namespace schema {\n\tdeclare interface SObjectsMap {\n ")
	...
	async generateTypings(sObjects:string[], sObjectTypingsGenerator:SObjectTypingsGenerator, typingsFolder:string, describesMap) {
		return Promise.all(sObjects.map(sObjectName => {
	...
	async generateSchemas(sObjects: string[], schemaGenerator: SchemaGenerator, typingsFolder: string, describesMap) {
		return Promise.all(sObjects.map(sObjectName => {
			const describe = describesMap.get(sObjectName.toLowerCase());
	...
	async fetchSObjectDefinitions(
		...
		const describes = await Promise.all(sObjects.map(sObject => this.org.getConnection().describe(sObject)))
