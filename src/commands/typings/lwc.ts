...
export default class GenerateLwcTypings extends SfdxCommand {
	...
	public static description = "Collective command for LWC typings generation. Generates apex, labels and stdlib typings and all required JsConfigs"
	...
	private async generateStdblib() {
		return new StandardLibraryGenerator().generateStandardLibrary(this.project, this.org.getConnection())
	}

	private async generateLabelsTypings() {
		return new LabelsTypingsGenerator().generateForAll(this.org.getConnection(), this.project)
